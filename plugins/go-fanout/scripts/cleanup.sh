...
if [ "$CG_ONLY" = "1" ]; then
  . "$(CDPATH='' cd -- "$(dirname -- "$0")" && pwd)/_cg.sh" 2>/dev/null || {
    echo "_cg.sh 를 못 읽었다 — 후보를 셀 수 없다." >&2; exit 2; }
  n_cand=0; n_skip=0; n_gone=0; n_fail=0
  while IFS="$(printf '\t')" read -r state wt age n; do
    [ -n "$state" ] || continue
    case "$state" in
      fresh)   echo "▸ 건너뜀 $wt — ${age}시간 전(기준 ${CLAUDE_CG_ORPHAN_HOURS:-6}시간 · 아직 쓰는 중일 수 있다)" ;;
      unknown) echo "▸ ⛔ 건너뜀 $wt — git status 를 못 돌렸다. 미커밋이 있는지 **모른다**"
               n_skip=$((n_skip+1)) ;;
      dirty)   echo "▸ ⛔ 건너뜀 $wt — 미커밋 ${n}건이다. 사람이 보고 정해야 한다"
               n_skip=$((n_skip+1)) ;;
      cand)
        n_cand=$((n_cand+1))
        if [ "$APPLY" = "1" ]; then
          if cg_remove "$wt"; then echo "▸ 지웠다 $wt (${age}시간 전 · 미커밋 0)"; n_gone=$((n_gone+1))
          else echo "▸ ⚠ 실패 $wt"; n_fail=$((n_fail+1)); fi
        else
          echo "▸ 후보 $wt — ${age}시간 전 · 미커밋 0"
        fi ;;
    esac
  done <<CGEOF
$(cg_candidates)
CGEOF
  echo
  if [ "$APPLY" = "1" ]; then
    echo "대조군 고아 정리: 지움 ${n_gone} · 실패 ${n_fail} · 건드리지 않음 ${n_skip}"
    [ "$n_fail" -eq 0 ] || exit 1
  else
    echo "대조군 고아 후보 ${n_cand}개 · 건드리지 않음 ${n_skip}개 (dry-run — 지우려면 --apply)"
  fi
  exit 0
fi
