  ...
  if grep -qE '^##[[:space:]]*병렬 배치' "$draft" 2>/dev/null; then
    ...
    if [ "$workers" -gt 0 ]; then
      ...
      if [ "$workers" -ge 2 ]; then
        if grep -qE '(^|[^A-Za-z])Q-P([^A-Za-z0-9]|$)' "$draft" 2>/dev/null; then
          printf '✅ 병렬 배치 협의 항목(Q-P) 있음 — 승인 전에 사용자가 배치를 본다.\n'
        else
          printf '⚠ 워커가 %s명인데 「## 결정 필요(승인 전)」에 **Q-P(병렬 배치)** 가 없다 — 병렬은 되돌리기 비싼 결정이다(워크트리 %s개·PR %s개). 파도별 워커·항목 · 파일 집합 교차 0 근거 · 공유 자원 · 대안(단독)을 Q-P 로 올리고 답 하나를 받아라(plan.md §6-b).\n' "$workers" "$workers" "$workers"
        fi
      fi
    else
